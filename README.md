...
mark_as_spam - Marks promotional, malicious, or junk emails as spam
search_company_policy - Searches internal policy documents using Pinecone
deny_request - Denies a customer request and logs the reason
Notes
Make sure your database is running and DATABASE_URL is correctly configured before starting the API.
Make sure your Gemini and Pinecone API keys are valid before running the agent or RAG upload script.
Do not commit your .env file to GitHub.
The local database tables are created automatically when the FastAPI app starts.
Repository Description
AI-powered email operations backend using FastAPI, Gemini, PostgreSQL, and Pinecone to process inbound support tickets with autonomous agent tools and RAG policy lookup.

For the GitHub repo’s small **Description/About** box, use only this:
